...
if __name__ == "__main__":
    ...
    if args.input:
        ...
        for vis_frame, pred in demo.run_on_video(video, args.confidence_threshold):
            ...
            # pred_m = pred_m[np.where(pred_c==3)]
            # print(len(pred_m))
            # print(pred_m.sum())



            # # Write all_bbox to file
            # print(len(pred_m))
            # txt_file = []
            # pred_empty = np.zeros((1080,1920))
            # for obj_idx in range(1,len(pred_m)):
            #     pred_objs = pred_m[np.where(pred_c==obj_idx)].copy()
            #     for pred_obj in pred_objs:

            #         height = pred_objs.shape[1]
            #         width = pred_objs.shape[2]
            #         pred_obj = pred_obj.astype(np.uint8)

            #         contours, hierarchy = cv2.findContours(pred_obj, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
            #         bb_boxes_obj = [cv2.boundingRect(cnt) for cnt in contours]

            #         # print("bb_boxes_all", bb_boxes_obj)
            #         # Writing to sample.json
            #         max_idx_table = 0
            #         max_area = 0
            #         bb_x, bb_y, bb_w, bb_h = 0, 0, 0, 0
            #         for bb_idx, bb_boxes in enumerate(bb_boxes_obj):
            #             x, y ,w, h = bb_boxes
            #             if w*h > max_area:
            #                 max_idx_table = bb_idx
            #                 max_area = w*h
            #                 bb_x, bb_y, bb_w, bb_h = x, y ,w, h
            #         if (bb_x, bb_y, bb_w, bb_h) != (0, 0 , 0 , 0):
            #             cv2.rectangle(pred_empty, (bb_x, bb_y) ,(bb_x+bb_w, bb_y+bb_h), (255, 0, 0), 4)
            #             print((bb_x, bb_y, bb_w, bb_h))
            #             txt_file.append(f"{str(obj_idx-1)} {str(bb_x/width)} {str(bb_y/height)} {str(bb_w/width)} {str(bb_h/height)}\n")
            # pred_empty = pred_empty.astype(np.uint8)
            # pred_empty = cv2.cvtColor(pred_empty, cv2.COLOR_GRAY2BGR)
            # # cv2.namedWindow("t2", cv2.WINDOW_NORMAL)
            # # cv2.imshow("t2", vis_frame)
            # # cv2.namedWindow("t", cv2.WINDOW_NORMAL)
            # # cv2.imshow("t", pred_empty)
            # # cv2.waitKey(0)
            # # exit()
            # # cv2.imwrite(f"./table-tennis/result/{frame_count}.png", pred_empty)
            # with open(f"./table-tennis/result/bbox_f-1/{frame_count}.txt", "w") as outfile:
            ...
            # print(bb_boxes_table[0])    # x, y, x+w, y+h
            ...
            # print(pred_m_pad_arr[0].shape)
            # print(pred_m_pad_arr[1].shape)
            ...
            for pad_idx, pred_m_pad in enumerate(pred_m_pad_arr):
                ...
                if len(bb_boxes_pad) == 0:
                    ...
                    if 0 <= bb_boxes_pad[max_idx][0] < table_center[0]:
                        ...
                        # cv2.imwrite('/home/yoson/SparseInst/official/SparseInst/table-tennis/label_mask/paddle_L/paddle_L_' + 'frame_' + str(frame_count) + '.png', label_mask_pad)
                        # cv2.namedWindow('pad_mask_L', 0)
                        # cv2.imshow('pad_mask_L', label_mask_pad)
                        # cv2.waitKey(1)
                        ...
                        # cv2.imwrite('/home/yoson/SparseInst/official/SparseInst/table-tennis/label_mask/paddle_R/paddle_R_' + 'frame_' + str(frame_count) + '.png', label_mask_pad_img)
                        # cv2.putText(label_mask_pad_img, "Frame: " + str(frame_count), (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255), 5, cv2.LINE_4)

                        half_image_pad = label_mask_pad_img[0:height, 960:width]            # crop half image

                        # print('=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=')
                        # left = int(bb_boxes_pad[max_idx][0])
                        # top = int(bb_boxes_pad[max_idx][1])
                        # right = int(bb_boxes_pad[max_idx][0] + (bb_boxes_pad[max_idx][2]))
                        # bottom = int(top + bb_boxes_pad[max_idx][3])
                        # print('bb_box_pad_lefttop: ', left, top, '\n' 'bb_box_pad_rightbottom: ', right, bottom)
                        # print('=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=')
                        # cropped_image_pad = label_mask_pad_img[top-50:bottom+50, left-50:right+50]        # crop bb_box range
                        # # Calculate the required padding
                        # pad_height = max(0, 640 - cropped_image_pad.shape[0])
                        # pad_width = max(0, 640 - cropped_image_pad.shape[1])
                        ...
                        # # Pad the image
                        # padded_image_pad = np.pad(cropped_image_pad, ((top_pad, bottom_pad), (left_pad, right_pad), (0, 0)), mode='constant')
                        # print(padded_image_pad.shape)

                        # cv2.namedWindow('pad_mask_R', 0)
                        # cv2.imshow('pad_mask_R', half_image_pad)
                        # cv2.waitKey(1)
            ...
            for person_idx, pred_m_person in enumerate(pred_m_person_arr):
                ...
                if len(bb_boxes_person) == 0:
                    ...
                    if 0 <= bb_boxes_person[max_idx][0] < table_center[0]:
                        ...
                        # cv2.imwrite('/home/yoson/SparseInst/official/SparseInst/table-tennis/label_mask/paddle_L/paddle_L_' + 'frame_' + str(frame_count) + '.png', label_mask_pad)
                        # cv2.namedWindow('person_mask_L', 0)
                        # cv2.imshow('person_mask_L', label_mask_person)
                        # cv2.waitKey(1)
                        ...
                        # print('=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=')
                        # left = int(bb_boxes_person[max_idx][0])
                        # top = int(bb_boxes_person[max_idx][1])
                        # right = int(bb_boxes_person[max_idx][0] + (bb_boxes_person[max_idx][2]))
                        # bottom = int(top + bb_boxes_person[max_idx][3])
                        # print('bb_box_person_lefttop: ', left, top, '\n' 'bb_box_person_rightbottom: ', right, bottom)
                        # print('=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=')
                        # cropped_image_person = label_mask_person_img[top-50:bottom+50, left-50:right+50]        # crop bb_box range
                        # # Calculate the required padding
                        # pad_height = max(0, 640 - cropped_image_person.shape[0])
                        # pad_width = max(0, 640 - cropped_image_person.shape[1])
                        # # Calculate the padding amounts for top, bottom, left, and right
                        # top_pad = pad_height // 2
                        # bottom_pad = pad_height - top_pad
                        # left_pad = pad_width // 2
                        # right_pad = pad_width - left_pad
                        # # Pad the image
                        # padded_image_person = np.pad(cropped_image_person, ((top_pad, bottom_pad), (left_pad, right_pad), (0, 0)), mode='constant')
                        # print(padded_image_person.shape)
                        # # cv2.imwrite('/home/yoson/SparseInst/official/SparseInst/table-tennis/result/org_img' + 'frame_' + str(frame_count) + '.png', cropped_image_person)
                        # # cv2.imwrite('/home/yoson/SparseInst/official/SparseInst/table-tennis/result/pad_img' + 'frame_' + str(frame_count) + '.png', padded_image_person)


                        # cv2.namedWindow('person_mask_R', 0)
                        # cv2.imshow('person_mask_R', half_image_person)
                        # cv2.waitKey(1)
            ...
            if args.output:
                ...
                # # Add frame number and paddle value to the DataFrame
                # frame_data = {'Frame': frame_count, 'Paddle_L_Pixel': paddle_L_str}
                # csv_data = csv_data.append(frame_data, ignore_index=True)
                ...
                # # Calculate the mean and variance of the data up to the current frame
                # paddle_L_area_mean = np.mean(paddle_L_area_list[:x_max])
                # paddle_L_area_variance = np.std(paddle_L_area_list[:x_max])
                # paddle_R_area_mean = np.mean(paddle_R_area_list[:x_max])
                # paddle_R_area_variance = np.std(paddle_R_area_list[:x_max])

                # # Normalize the data using (xi - mean) / variance formula
                # paddle_L_area_normalized = (paddle_L_area_list[:x_max] - paddle_L_area_mean) / paddle_L_area_variance
                # paddle_R_area_normalized = (paddle_R_area_list[:x_max] - paddle_R_area_mean) / paddle_R_area_variance

                # # Scale the normalized data to the range of 0 to 1
                # paddle_L_area_scaled = (paddle_L_area_normalized - np.min(paddle_L_area_normalized)) / (np.max(paddle_L_area_normalized) - np.min(paddle_L_area_normalized))
                # paddle_R_area_scaled = (paddle_R_area_normalized - np.min(paddle_R_area_normalized)) / (np.max(paddle_R_area_normalized) - np.min(paddle_R_area_normalized))

                # #　smooth the data
                # paddle_L_area_scaled_temp = paddle_L_area_scaled
                # # if (len(paddle_L_area_scaled)) > 12:
                # #     from scipy.signal import savgol_filter

                # #     paddle_L_area_scaled_temp = savgol_filter(paddle_L_area_scaled, 11, 3)                  # window size 51, polynomial order 3

                # # Initialize the plot
                # plt.figure(figsize=(plot_width / 100, plot_height / 100))
                # plt.plot(range(x_min, x_max), paddle_L_area_scaled_temp, 'ro-', label='Left Paddle', markersize=1)    # paddle_L_area, blue
                # # plt.plot(range(x_min, x_max), paddle_R_area_scaled, 'go-', label='Right Paddle', markersize=1)   # paddle_R_area, green
                # plt.xlim(x_min, x_max)
                # plt.ylim(0, 1)  # Adjust the y-axis limits as needed

                # print('////////////////////////////////////////////////////////////////////////////////////////////////')
                # print('paddle_L_area_normalized: ' ,paddle_L_area_normalized)
                # print('paddle_R_area_normalized: ' ,paddle_R_area_normalized)



                # # Extract the relevant data points for plotting
                # x_data = range(x_min, x_max)
                # y_data_L = paddle_L_area_list[x_min:x_max + 1]
                # y_data_R = paddle_R_area_list[x_min:x_max + 1]
                # plt.plot(x_data, y_data_L, 'ro-', label='Left Paddle', markersize=1)            # paddle_L_area, blue
                # plt.plot(x_data, y_data_R, 'go-', label='Right Paddle', markersize=1)           # paddle_R_area, green
                # plt.xlim(x_min, x_max)

                # # Plot all the previously stored paddle areas
                # plt.plot(range(frame_count), paddle_L_area_list, 'ro-', label='Left Paddle', markersize=1)    # paddle_L_area, blue
                # plt.plot(range(frame_count), paddle_R_area_list, 'go-', label='Right Paddle', markersize=1)   # paddle_R_area, green
                # plt.xlim(0, frame_count)

                # plt.ylim(0, 3000)
                # plt.legend(loc='upper right')
                # # Set x-axis and y-axis labels
                # plt.xlabel('Frame')                      # Frame     Time
                # plt.ylabel('Paddle Pixels Area')
                ...
                ##################################################################################################################
                # # Calculate the time in seconds based on the frame count and frame rate
                # frame_rate = 60         # Adjust this value with actual video frame rate
                # time_min = x_min / frame_rate
                # time_max = x_max / frame_rate
                # Extract the relevant data points for plotting
                # x_data = np.linspace(time_min, time_max, x_max - x_min)
                # y_data_L = paddle_L_area_list[x_min:x_max+1]
                # y_data_R = paddle_R_area_list[x_min:x_max+1]
                # plt.plot(x_data, y_data_L, 'ro-', label='Left Paddle', markersize=1)            # paddle_L_area, red
                # plt.plot(x_data, y_data_R, 'go-', label='Right Paddle', markersize=1)           # paddle_R_area, green
                # plt.xlim(time_min, time_max)
                # # Format the x-axis labels as minutes and seconds
                # def format_time(x, pos=None):
                #     minutes = int(x / 60)
                #     seconds = int(x % 60)
                #     return f"{minutes}:{seconds:02d}"
                # # Create a formatter object using the custom format function
                # time_formatter = ticker.FuncFormatter(format_time)
                ...
                # Display the frame_with_plot image
                # cv2.namedWindow('result', 0)
                # cv2.imshow('result', vis_frame_with_plot)
                # Write the frame with the plot to the video file
                ...
                ################################################################
                # # Plot bbox_pad_center
                # # Clear the previous plot
                # plt.clf()
                # fig = plt.gcf()
                # # Set the desired plot size
                # plot_width_2 = 720               # 720           1920
                # plot_height_2 = 480               # 480
                # plot_x_2 = 0                      # 1200      0
                # plot_y_2 = 0
                # # Initialize the plot
                # plt.figure(figsize=(plot_width_2 / 100, plot_height_2 / 100))


                #  # Plot all the previously stored paddle bb_box center
                # print('*-*-*-*--*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*')
                # print("bb_boxes_pad_R_center_list: ", bb_boxes_pad_L_center_list)
                # plt.plot([i[0] for i in bb_boxes_pad_R_center_list], [i[1] for i in bb_boxes_pad_R_center_list], 'ro-', label='Right paddle center', markersize=1)
                # plt.xlim(0, 1920)
                # plt.ylim(0, 1080)

                # plt.legend(loc='upper left')
                # # Set x-axis and y-axis labels
                # plt.xlabel('x')                      # Frame     Time
                # plt.ylabel('y')

                # # Convert the plot to an image
                # fig.set_size_inches(plot_width_2 / 100, plot_height_2 / 100)
                # fig.canvas.draw()
                # plot_img = np.array(fig.canvas.renderer.buffer_rgba())
                # # Resize the plot image to the desired size
                # plot_img_resized = cv2.resize(plot_img, (plot_width_2, plot_height_2))
                # # Convert the plot image to have the same number of color channels as the frame image
                # plot_img_resized_rgb = cv2.cvtColor(plot_img_resized, cv2.COLOR_RGBA2RGB)
                ...
                # # Overlay the plot image on top of the frame
                # vis_frame_with_plot[plot_y_2:plot_y_2 + plot_height_2, plot_x_2:plot_x_2 + plot_width_2] = plot_img_resized_rgb
                ...
                for i in range(frame_min_pad_L, min(frame_max, len(bb_boxes_pad_L_center_list))):
                    ...
                    print('center', center)
                    # Plot for the second subplot: Paddle_route
                    # color_map = plt.cm.get_cmap('Reds')  # Get the colormap
                    # rgba_color = color_map(gradient)  # Get the color based on the gradient
                    # area_table[1].scatter([center[0]], [1080-center[1]], color=rgba_color, marker='o', s=10, cmap='Blues')
                ...
                # for i in range(frame_min, min(frame_max, len(bb_boxes_pad_R_center_list))):
                #     center = (int(bb_boxes_pad_R_center_list[i][0]), int(bb_boxes_pad_R_center_list[i][1]))
                #     # Calculate the color for the current frame
                #     if i == frame_count - 1:
                #         color = (0, 255, 0)  # Lightest green for the current frame
                #     else:
                #         gradient = (frame_count - i - 1) / 29.0
                #         color_component = int(255 - (gradient * 200))                 # Decreasing red component for gradient        color = (0, 0, 255)   # BGR
                #         color = (0, color_component, 0)         # Light red to dark red gradient
                ...
                # cv2.putText(vis_frame, "FPS: " + str(float(frames_per_second)), (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 0), 5, cv2.LINE_4)
            ...
            # csv_data.to_excel('/home/yoson/SparseInst/official/SparseInst/table-tennis/result/pad_area.xlsx', index=False)


            # end = time.perf_counter() - start_time_perf
            # durations.append(end)
            # print(durations)
            print("=============================================================")
            # print("process: [{}/{}] fps: {:.3f}".format(frame_count, num_frames, 1/(np.mean(durations))))
            # print("process: [{}/{}] fps: {:.3f}".format(frame_count, num_frames, 1/(np.mean(durations[-1])/frame_count)  ))
